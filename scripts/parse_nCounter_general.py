...
def plot_gene(gene_details_map, gene, user_id, report_uuid):
    ...
    errors = [[0, gene_details_map[gene]["mock"]["std"]/2], [0, gene_details_map[gene]["cond1"]["std"]/2],
              [0, gene_details_map[gene]["cond2"]["std"]/2]]

    labels = ('mock', 'cond1', 'cond2')
    fig, ax = plt.subplots()
    rect = plt.bar(x_pos, means, 0.7, color='lightskyblue', align='center', linewidth=0, )

    plotline1, caplines1, barlinecols1 = plt.errorbar(x_pos, means, yerr=errors, lolims=True, ls='None', color='black', barsabove=True)
    ...
    label_significance(x_pos[1], gene_details_map[gene]["mock"]["fold_change"], gene_details_map[gene]["cond1"]["fold_change"], errors[1], ymax)
    label_significance(x_pos[2], gene_details_map[gene]["mock"]["fold_change"], gene_details_map[gene]["cond2"]["fold_change"], errors[2], ymax)
